...
public class Solution {
    ...
    public static void main(String[] args)throws IOException {
        ...
        BufferedReader fileBufferedReaqder = new BufferedReader(new FileReader(bufferedReader.readLine()));
        bufferedReader.close();

        while (fileBufferedReaqder.ready()){
            String tmp = fileBufferedReaqder.readLine();
            List<String> buffer = Arrays.asList(tmp.split(" "));
            int count = 0;
            for (String s:
                 words) {
                count+= Collections.frequency(buffer,s);
            }
            if (count == 2){
